...
The result is the probability that the simulated values are larger than the sample values. Number of iteration: 1000

## Figure 1 - Joint distribution of the coefficients ($b_r$ and $b_d$) and their t-statistics

Number of iteration: 1000. The round blue dot is the sample values, the diamond red dot is the null used to simulate data.
...
Number of iteration: 1000. The vertical line is the sample value.
...
Number of iteration: 1000. The dashed line ($b_d$) is $b_r = 1- \rho\phi + \hat{b_d}$, the solid line $b_r^{lr}$ is $b_r/(1-\rho\phi) = \hat{b_r}/(1-\rho\hat{\phi})$.
...
The values in the table are the probability that the coefficients are larger than the sample values, with the number of iteration is 1000.
