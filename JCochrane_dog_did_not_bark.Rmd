# The Dog That Did Not Bark: A Defense of Return Predictability (Replication)

```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Check packages and install if needed
  check_packages = c("readxl", "xtable", "data.table", "dyn", "kableExtra", "mvtnorm", "ggplot2")
...
#Get relevant data
  #Edit data source here
    data_link = "D:/University of Kent/01. Lectures/SPR - EC843 - Financial Econometrics/R/GWAnnual.csv"
...
The result is quite similar to paper's result. The coefficients of dividend growth and log dividend growth are negative as expected, as high dividend/price ratio implies low price, which indicates that the dividend in the future would be low.
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
#Monte-Carlo Simulation for return & dividend growth
  #Simulation function
...
The result is the probability that the simulated values are larger than the sample values. Number of iteration: 5000

## Figure 1 - Joint distribution of the coefficients ($b_r$ and $b_d$) and their t-statistics

Number of iteration: 5000. The round blue dot is the sample values, the diamond red dot is the null used to simulate data.
...
The p-values are retrieved from the simulation with known $b_r = 0$ and $\phi = 0.940$. I did not change the selection of the coefficients to reduce calculation time, so the p-value is not a range.

## Figure 2 - Distribution of the $b_r^{lr}$

Number of iteration: 5000. The vertical line is the sample value.
...
## Figure 3 - Joint distribution of $b_r$ and $\phi$

Number of iteration: 5000. The dashed line ($b_d$) is $b_r = 1- \rho\phi + \hat{b_d}$, the solid line $b_r^{lr}$ is $b_r/(1-\rho\phi) = \hat{b_r}/(1-\rho\hat{\phi})$.
...
The values in the table are the probability that the coefficients are larger than the sample values, with the number of iteration is 5000.

## Figure 5 - Joint distribution of $b_r$ and $\phi$ estimates, together with regions implied by long-run regressions
